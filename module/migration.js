...
export const migrateWorld = async function () {
  ui.notifications.info(
    `Applying Castles & Crusades System Migration for version ${game.system.data.version}. Please be patient and do not close your game or shut down your server.`,
    { permanent: true }
  );
  ...
  ui.notifications.info(
    `DnD5E System Migration to version ${game.system.data.version} completed!`,
    { permanent: true }
  );
};
...
export const migrateCompendium = async function (pack) {
  ...
  for (const ent of content) {
    ...
    try {
      ...
      err.message = `Failed cnc system migration for entity ${ent.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
export async function purgeFlags(pack) {
  const cleanFlags = (flags) => {
    const flags5e = flags.cnc || null;
    return flags5e ? { cnc: flags5e } : {};
  };
  await pack.configure({ locked: false });
  const content = await pack.getContent();
