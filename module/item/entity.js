...
export default class Item5e extends Item {
      ...
      // Weapons
        ...
        if (["simpleM", "martialM"].includes(wt)) {
          if (itemData.properties.fin === true) {
            // Finesse weapons
            return actorData.abilities.dex.mod >= actorData.abilities.str.mod
              ? "dex"
              : "str";
          }
          return "str";
   ...
   */
  get hasLimitedUses() {
    const chg = this.data.data.recharge || {};
  ...
  async displayCard({ rollMode, createMessage = true } = {}) {
    ...
    const templateType = ["tool"].includes(this.data.type)
      ? this.data.type
      : "item";
    const template = `systems/cnc/templates/chat/${templateType}-card.html`;
    ...
    const rollConfig = mergeObject(
      {
        ...
        messageData: { "flags.cnc.roll": { type: "attack", itemId: this.id } },
      },
      options
    );
  ...
  rollDamage({
    ...
    if (itemData.actionType === "mwak") {
      rollConfig.criticalBonusDice =
        this.actor.getFlag("cnc", "meleeCriticalDamageDice") ?? 0;
    }
    ...
    const rollConfig = mergeObject(
      {
        parts: parts,
        data: rollData,
        template: "systems/cnc/templates/chat/tool-roll-dialog.html",
        title: title,
        speaker: ChatMessage.getSpeaker({ actor: this.actor }),
        flavor: title,
        dialogOptions: {
        ...
        messageData: { "flags.cnc.roll": { type: "tool", itemId: this.id } },
      },
      options
    );
    rollConfig.event = options.event;
  ...
  static _getChatCardTargets(card) {
    ...
    if (!targets.length)
      ui.notifications.warn(game.i18n.localize("CNC.ActionWarningNoToken"));
    return targets;
  }
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = CONFIG.CNC.spellScrollIds[level];
