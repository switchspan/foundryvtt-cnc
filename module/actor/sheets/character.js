...
export default class ActorSheetCncCharacter extends ActorSheetCnc {
  ...
  getData() {
    ...
    // Resources
    sheetData.resources = ["primary", "secondary", "tertiary"].reduce(
      (arr, r) => {
    ...
    // Experience Tracking
    sheetData.disableExperience = game.settings.get(
      "cnc",
      "disableExperienceTracking"
    );
    sheetData.classLabels = this.actor.itemTypes.class
      .map((c) => c.name)
      .join(", ");
  ...
  _prepareItems(data) {
    ...
    const inventory = {
      weapon: {
        label: "CNC.ItemTypeWeaponPl",
        items: [],
        dataset: { type: "weapon" },
      },
      equipment: {
        label: "CNC.ItemTypeEquipmentPl",
        items: [],
        dataset: { type: "equipment" },
      },
      consumable: {
        label: "CNC.ItemTypeConsumablePl",
        items: [],
        dataset: { type: "consumable" },
      },
      tool: {
        label: "CNC.ItemTypeToolPl",
        items: [],
        dataset: { type: "tool" },
      },
      backpack: {
        label: "CNC.ItemTypeContainerPl",
        items: [],
        dataset: { type: "backpack" },
      },
      loot: {
        label: "CNC.ItemTypeLootPl",
        items: [],
        dataset: { type: "loot" },
      },
    };
    ...
    let [items, spells, feats, classes] = data.items.reduce(
      (arr, item) => {
        ...
        item.isStack =
          Number.isNumeric(item.data.quantity) && item.data.quantity !== 1;
        item.attunement = {
          1: {
            icon: "fa-sun",
            cls: "not-attuned",
    ...
    const features = {
      ...
      active: {
        label: "CNC.FeatureActive",
        items: [],
        hasActions: true,
        dataset: { type: "feat", "activation.type": "action" },
      },
      passive: {
        label: "CNC.FeaturePassive",
        items: [],
        hasActions: false,
        dataset: { type: "feat" },
      },
  ...
  async _onDropItemCreate(itemData) {
    ...
    if (itemData.type === "class") {
      const cls = this.actor.itemTypes.class.find(
        (c) => c.name === itemData.name
      );
      const priorLevel = cls?.data.data.levels ?? 0;
      if (cls) {
        const next = Math.min(
          priorLevel + 1,
          20 + priorLevel - this.actor.data.data.details.level
        );
        if (next > priorLevel) {
