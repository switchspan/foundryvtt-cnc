...
export default class Actor5e extends Actor {
  ...
  _computeEncumbrance(actorData) {
    ...
    const max = actorData.data.abilities.str.value * CONFIG.CNC.encumbrance.strMultiplier * mod;
  ...
  rollSkill(skillId, options={}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("CNC.SkillPromptTitle", {skill: CONFIG.CNC.skills[skillId]}),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    if ( feats.remarkableAthlete && CNC.characterFlags.remarkableAthlete.abilities.includes(abilityId) ) {
  ...
  async rollDeathSave(options={}) {
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        await ChatMessage.create({content: game.i18n.format("CNC.DeathSaveCriticalSuccess", {name: this.name}), speaker});
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        await ChatMessage.create({content: game.i18n.format("CNC.DeathSaveSuccess", {name: this.name}), speaker});
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        await ChatMessage.create({content: game.i18n.format("CNC.DeathSaveFailure", {name: this.name}), speaker});
      }
    }
  ...
  async shortRest({dialog=true, chat=true, autoHD=false, autoHDThreshold=3}={}) {
    ...
    if ( chat ) {
      ...
      switch (game.settings.get("cnc", "restVariant")) {
        case 'normal': restFlavor = game.i18n.localize("CNC.ShortRestNormal"); break;
        case 'gritty': restFlavor = game.i18n.localize(newDay ? "CNC.ShortRestOvernight" : "CNC.ShortRestGritty"); break;
        case 'epic':  restFlavor = game.i18n.localize("CNC.ShortRestEpic"); break;
      }
  ...
  async longRest({dialog=true, chat=true, newDay=true}={}) {
    ...
    let restFlavor;
    switch (game.settings.get("cnc", "restVariant")) {
      case 'normal': restFlavor = game.i18n.localize(newDay ? "CNC.LongRestOvernight" : "CNC.LongRestNormal"); break;
      case 'gritty': restFlavor = game.i18n.localize("CNC.LongRestGritty"); break;
      case 'epic':  restFlavor = game.i18n.localize("CNC.LongRestEpic"); break;
    }
    ...
    if ( chat ) {
      ...
      else if ((dhp !== 0) && (dhd === 0)) lrMessage = "CNC.LongRestResultHitPoints";
