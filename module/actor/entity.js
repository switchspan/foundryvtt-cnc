...
export default class ActorCnc extends Actor {
  ...
  prepareDerivedData() {
    ...
    const dcBonus = Number.isNumeric(data.bonuses?.spell?.dc)
      ? parseInt(data.bonuses.spell.dc)
      : 0;
  ...
  static async getClassFeatures({
    ...
    const clsConfig = CONFIG.CNC.classFeatures[className];
    if (!clsConfig) return [];
  ...
  async updateEmbeddedEntity(embeddedName, data, options = {}) {
    const createItems =
      embeddedName === "OwnedItem" ? await this._createClassFeatures(data) : [];
    const updated = await super.updateEmbeddedEntity(
      embeddedName,
      data,
      options
    );
    if (createItems.length)
      await this.createEmbeddedEntity("OwnedItem", createItems);
    return updated;
  }
  ...
  async _createClassFeatures(updated) {
    const toCreate = [];
    for (const u of updated instanceof Array ? updated : [updated]) {
      ...
      const config = {
        ...
        subclassName:
          getProperty(updateData, "data.subclass") || item.data.data.subclass,
  ...
  _prepareNPCData(actorData) {
    ...
    if (
      ...
      !Number.isNumeric(data.details.spellLevel)
    ) {
  ...
  _prepareSkills(actorData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = CNC.characterFlags;
    const athlete = flags.remarkableAthlete;
    const joat = flags.jackOfAllTrades;
    const observant = flags.observantFeat;
    const skillBonus = Number.isNumeric(bonuses.skill)
      ? parseInt(bonuses.skill)
      : 0;
    ...
    for (const [id, skl] of Object.entries(data.skills)) {
      ...
      if (
        athlete &&
        skl.value === 0 &&
        feats.remarkableAthlete.abilities.includes(skl.ability)
      ) {
        multi = 0.5;
        round = Math.ceil;
      }
      ...
      const passive =
        observant && feats.observantFeat.skills.includes(id) ? 5 : 0;
  ...
  _computeSpellcastingDC(actorData) {
    ...
    const data = actorData.data;
    data.attributes.spelldc = data.attributes.spellcasting
      ? data.abilities[data.attributes.spellcasting].dc
      : 10;
  ...
  _computeSpellcastingProgression(actorData) {
    ...
    if (
      !isNPC &&
      isSingleClass &&
      ["half", "third"].includes(caster.data.spellcasting)
    ) {
    ...
    const slots = CNC.SPELL_SLOT_TABLE[levels - 1] || [];
    for (const [n, lvl] of Object.entries(spells)) {
      const i = parseInt(n.slice(-1));
      if (Number.isNaN(i)) continue;
      if (Number.isNumeric(lvl.override))
        lvl.max = Math.max(parseInt(lvl.override), 1);
    ...
    if (pl === 0 && isNPC && Number.isNumeric(spells.pact.override))
      pl = actorData.data.details.spellLevel;

    // Determine the number of Warlock pact slots per level
    if (pl > 0) {
      spells.pact.level = Math.ceil(Math.min(10, pl) / 2);
      if (Number.isNumeric(spells.pact.override))
        spells.pact.max = Math.max(parseInt(spells.pact.override), 1);
      else
  ...
  _computeEncumbrance(actorData) {
    ...
    if (game.settings.get("cnc", "currencyWeight")) {
    ...
    const max =
      ...
      CONFIG.CNC.encumbrance.strMultiplier *
      mod;
  ...
  async update(data, options = {}) {
    ...
    if (newSize && newSize !== getProperty(this.data, "data.traits.size")) {
      const size = CONFIG.CNC.tokenSizes[newSize];
    ...
    if (
      this.data.data.attributes.hp.value <= 0 &&
      getProperty(data, "data.attributes.hp.value") > 0
    ) {
  ...
  async createEmbeddedEntity(embeddedName, itemData, options = {}) {
    ...
    if (embeddedName === "OwnedItem")
      this._preCreateOwnedItem(itemData, options);
    ...
    switch (itemData.type) {
      case "weapon":
        ...
        if (!isNPC) {
          ...
          const actorWeaponProfs =
            this.data.data.traits?.weaponProf?.value || [];
          hasWeaponProf =
            weaponProf === true || actorWeaponProfs.includes(weaponProf);
        }
  ...
  rollSkill(skillId, options = {}) {
    ...
    const reliableTalent =
      skl.value >= 1 && this.getFlag("cnc", "reliableTalent");

    // Roll and return
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("CNC.SkillPromptTitle", {
        skill: CONFIG.CNC.skills[skillId],
      }),
      halflingLucky: this.getFlag("cnc", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: { "flags.cnc.roll": { type: "skill", skillId } },
    });
    rollData.speaker =
  ...
  rollAbility(abilityId, options = {}) {
    ...
    new Dialog({
      title: game.i18n.format("CNC.AbilityPromptTitle", { ability: label }),
      content: `<p>${game.i18n.format("CNC.AbilityPromptText", {
        ability: label,
      })}</p>`,
      buttons: {
        test: {
  ...
  rollAbilityTest(abilityId, options = {}) {
    ...
    const feats = this.data.flags.cnc || {};
    if (
      feats.remarkableAthlete &&
      CNC.characterFlags.remarkableAthlete.abilities.includes(abilityId)
    ) {
    ...
    const rollData = mergeObject(options, {
      ...
      messageData: { "flags.cnc.roll": { type: "ability", abilityId } },
    });
    rollData.speaker =
      options.speaker || ChatMessage.getSpeaker({ actor: this });
    return d20Roll(rollData);
  ...
  rollAbilitySave(abilityId, options = {}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("CNC.SavePromptTitle", { ability: label }),
      halflingLucky: this.getFlag("cnc", "halflingLucky"),
      messageData: { "flags.cnc.roll": { type: "save", abilityId } },
    });
    rollData.speaker =
      options.speaker || ChatMessage.getSpeaker({ actor: this });
    return d20Roll(rollData);
  ...
  async rollDeathSave(options = {}) {
    ...
    const rollData = mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.localize("CNC.DeathSavingThrow"),
      speaker: speaker,
      halflingLucky: this.getFlag("cnc", "halflingLucky"),
      targetValue: 10,
      messageData: { "flags.cnc.roll": { type: "death" } },
    });
    ...
    if (success) {
      ...
      if (d20 === 20) {
        ...
        await ChatMessage.create({
          content: game.i18n.format("CNC.DeathSaveCriticalSuccess", {
            name: this.name,
          }),
          speaker,
        });
      ...
      // 3 Successes = survive and reset checks
        ...
        await ChatMessage.create({
          content: game.i18n.format("CNC.DeathSaveSuccess", {
            name: this.name,
          }),
          speaker,
        });
    ...
    // Save failure
      ...
      const failures = (death.failure || 0) + (d20 === 1 ? 2 : 1);
      await this.update({
      ...
      if (failures >= 3) {
        // 3 Failures = death
        await ChatMessage.create({
          content: game.i18n.format("CNC.DeathSaveFailure", {
            name: this.name,
          }),
          speaker,
        });
  ...
  async rollHitDie(denomination, { dialog = true } = {}) {
    ...
    if (!denomination) {
      cls = this.itemTypes.class.find(
        (c) => c.data.data.hitDiceUsed < c.data.data.levels
      );
      if (!cls) return null;
    ...
    // Otherwise locate a class (if any) which has an available hit die of the requested denomination
      ...
      cls = this.items.find((i) => {
        const d = i.data.data;
        return (
          d.hitDice === denomination && (d.hitDiceUsed || 0) < (d.levels || 1)
        );
      });
    }
    ...
    if (!cls) {
      ui.notifications.error(
        game.i18n.format("CNC.HitDiceWarn", {
          name: this.name,
    ...
    const title = game.i18n.localize("CNC.HitDiceRoll");
    ...
    const roll = await damageRoll({
      ...
      messageData: { "flags.cnc.roll": { type: "hitDie" } },
    });
    if (!roll) return null;
  ...
  async shortRest({
    ...
    const updateData = {};
    for (const [k, r] of Object.entries(this.data.data.resources)) {
      if (r.max && r.sr) {
    ...
    const items = this.items.filter(
      (item) =>
        item.data.data.uses && recovery.includes(item.data.data.uses.per)
    );
    ...
    if (chat) {
      ...
      switch (game.settings.get("cnc", "restVariant")) {
        case "normal":
          restFlavor = game.i18n.localize("CNC.ShortRestNormal");
          break;
        case "gritty":
          restFlavor = game.i18n.localize(
            newDay ? "CNC.ShortRestOvernight" : "CNC.ShortRestGritty"
          );
          break;
        case "epic":
          restFlavor = game.i18n.localize("CNC.ShortRestEpic");
          break;
      }

      // Summarize the health effects
      let srMessage = "CNC.ShortRestResultShort";
      if (dhd !== 0 && dhp !== 0) srMessage = "CNC.ShortRestResult";
      ...
      ChatMessage.create({
        ...
        flavor: restFlavor,
        content: game.i18n.format(srMessage, {
          name: this.name,
          dice: -dhd,
          health: dhp,
        }),
  ...
  async longRest({ dialog = true, chat = true, newDay = true } = {}) {
    ...
    // Recover spell slots
    for (const [k, v] of Object.entries(data.spells)) {
    ...
    const updateItems = this.items
      ...
      .sort((a, b) => {
        const da = parseInt(a.data.data.hitDice.slice(1)) || 0;
        const db = parseInt(b.data.data.hitDice.slice(1)) || 0;
        return db - da;
      })
      .reduce((updates, item) => {
        const d = item.data.data;
        if (recoverHD > 0 && d.hitDiceUsed > 0) {
          const delta = Math.min(d.hitDiceUsed || 0, recoverHD);
          recoverHD -= delta;
          dhd += delta;
          updates.push({
            _id: item.id,
    ...
    switch (game.settings.get("cnc", "restVariant")) {
      case "normal":
        restFlavor = game.i18n.localize(
          newDay ? "CNC.LongRestOvernight" : "CNC.LongRestNormal"
        );
        break;
      case "gritty":
        restFlavor = game.i18n.localize("CNC.LongRestGritty");
        break;
      case "epic":
        restFlavor = game.i18n.localize("CNC.LongRestEpic");
        break;
    }

    // Determine the chat message to display
    if (chat) {
      let lrMessage = "CNC.LongRestResultShort";
      if (dhp !== 0 && dhd !== 0) lrMessage = "CNC.LongRestResult";
      else if (dhp !== 0 && dhd === 0)
        lrMessage = "CNC.LongRestResultHitPoints";
      else if (dhp === 0 && dhd !== 0) lrMessage = "CNC.LongRestResultHitDice";
      ChatMessage.create({
        user: game.user._id,
        speaker: { actor: this, alias: this.name },
        flavor: restFlavor,
        content: game.i18n.format(lrMessage, {
          name: this.name,
          health: dhp,
          dice: dhd,
        }),
  ...
  convertCurrency() {
    ...
    const convert = CONFIG.CNC.currencyConversion;
  ...
  async transformInto(
    ...
    if (keepSkills) d.data.skills = o.data.skills;
      ...
      for (const [k, s] of Object.entries(d.data.skills)) {
    ...
    // Set new data flags
    if (!this.isPolymorphed || !d.flags.cnc.originalActor)
      d.flags.cnc.originalActor = this.id;
  ...
  async revertOriginalForm() {
    ...
    if (!this.owner) {
      return ui.notifications.warn(
        game.i18n.localize("CNC.PolymorphRevertWarn")
      );
    }
